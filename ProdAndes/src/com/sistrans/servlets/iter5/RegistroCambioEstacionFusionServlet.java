...
/**
 * Servlet implementation class RegistroCambioEstacionFusionServlet
 */
@WebServlet("/registro/fusion/cambioEstaciones.html")
public class RegistroCambioEstacionFusionServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RegistroCambioEstacionFusionServlet() {
        super();
	...
	private void printHeader(PrintWriter salida) {
		...
		salida.println("            	<h1>Cambio de estado FUSION</h1>");
	}
		...
		if(etapas.size() == 0) {
			...
			for(EtapadeProduccion e : etapas) {
				...
				if(e.getEstado()) {
					...
					salida.println("							<td><button class=\"btn btn-danger\"><a style=\"text-decoration:none; color:black;\" href=\"/ProdAndes/registro/fusion/cambioEstaciones.html?submit=Desactivar&id=" + e.getNum() + "\">Desactivar</a></button></td>");
				} else {
					salida.println("							<td>Inactiva</td>");
					salida.println("							<td><button class=\"btn btn-success\"><a style=\"text-decoration:none; color:black;\" href=\"/ProdAndes/registro/fusion/cambioEstaciones.html?submit=Activar&id=" + e.getNum() + "\">Activar</a></button></td>");
				}
				salida.println("                        </tr>");
			}
			salida.println("                    </tbody>");
